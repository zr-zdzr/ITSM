...
**Google Workspace SSO implemented, shipping dormant** — it activates itself when
`GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` exist at boot (§6); until then sign-in stays local
email + password. Phase 2 (hardware approvals) and in-app/email notifications remain design-only.
...
existing `inv_requests` flow already covers them (§7). Under active SSO, local credentials are
removed from every user-facing path; the single exception is the env-seeded break-glass admin
(`ADMIN_USERNAME`/`ADMIN_PASSWORD`) so a Google outage can never lock the org out. (The original
"Google OAuth @bykea.com" claims in this repo were never real and were removed in PRs #11–#14 —
this section is the first genuine implementation.)
...
| Hardware upgrade two-tier approval    | No department-head/manager concept anywhere (`heads`/`sub_heads` are _cost_ heads)                                                         | Phase 2: `department_heads`, `approval_roles`, `ticket_approvals`, IT Verdict (§2b)             |
...
**Google Workspace SSO — IMPLEMENTED (dormant until credentials exist).**

The full flow is built and ships disabled; the mode is decided entirely by the environment:
`googleEnabled()` (`backend/src/utils/googleAuth.js`) is true iff `GOOGLE_CLIENT_ID` and
`GOOGLE_CLIENT_SECRET` are both set at boot. No code change is involved in the cutover.

|                                 | Dormant (no credentials)                                          | SSO active (credentials set)                                                                                                                                                                                             |
| ------------------------------- | ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Sign-in                         | local email + password for everyone                               | **Google only**, restricted server-side to `@GOOGLE_ALLOWED_DOMAIN` (default `bykea.com`); the `hd` param is sent but treated as advisory                                                                                |
| Break-glass                     | n/a                                                               | the `ADMIN_USERNAME` account may still use `ADMIN_PASSWORD` (hidden behind "Administrator sign-in" on the login page)                                                                                                    |
| Other password endpoints        | change-password, admin reset, temp-password provisioning all work | all answer 400 — passwords are managed by Google; provisioning creates **passwordless** rows (`password_hash NULL`)                                                                                                      |
| Identity resolution on callback | —                                                                 | match by `google_id` → else link existing user by verified email (sets `google_id`, clears `must_change_password`) → else **auto-provision** a `role='employee'` account, attached to the matching employee row by email |
| Deactivated accounts            | blocked at login                                                  | blocked at callback (`?error=deactivated`)                                                                                                                                                                               |
| Login page                      | password form                                                     | "Sign in with Google" button (from `GET /auth/config`), SSO errors surfaced via `/login?error=<slug>`                                                                                                                    |

Schema note: `users.google_id` (the Google `sub` claim) existed from day one and is now actually
used; `password_hash` is retained as a column because dormant mode and the break-glass admin need
it — under active SSO no staff row ever gets one, and legacy hashes become inert (the login route
refuses non-admin password logins regardless).

**Activation checklist (the only manual steps):**

1. Create/verify the OAuth client in Google Cloud Console (consent screen: _Internal_); authorized
   redirect URI = `https://<host>/auth/google/callback`.
2. Set `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_CALLBACK_URL` in `.env` **and uncomment
   the three pass-through lines in `docker-compose.yml`** (deliberately commented so stale
   credentials can never flip the mode by accident).
3. `docker compose up -d --build backend` — done. To revert, re-comment and rebuild.
