...
const BUYABLE = {
  systems:         { table: 'systems',         permModule: 'systems', hasAssignee: true  },
  mobiles:         { table: 'mobiles',         permModule: 'mobiles', hasAssignee: true  },
  network_devices: { table: 'network_devices', permModule: 'network', hasAssignee: false },
};
...
router.get('/', requireAuth, perm('systems', 'read'), async (req, res, next) => {
  try {
    ...
    let where = 'WHERE 1=1';
    const params = [];
    if (asset_type)  { params.push(asset_type);  where += ` AND ap.asset_type=$${params.length}`; }
    if (employee_id) { params.push(employee_id); where += ` AND ap.buyer_employee_id=$${params.length}`; }
    const r = await db.query(`${PURCHASE_SELECT} ${where} ORDER BY ap.sale_date DESC, ap.id DESC`, params);
    res.json(r.rows);
  } catch (e) { next(e); }
});
...
router.post('/', requireAuth, async (req, res, next) => {
  const {
    asset_type, asset_id, buyer_employee_id,
    sale_price_pkr, book_value_pkr, sale_date,
    invoice_number, payment_reference, notes,
  } = req.body;

  const cfg = BUYABLE[asset_type];
  if (!cfg) return res.status(400).json({ error: `Unsupported asset_type for buyout: ${asset_type}` });
  if (!asset_id) return res.status(400).json({ error: 'asset_id is required' });
  if (!buyer_employee_id) return res.status(400).json({ error: 'buyer_employee_id is required' });
  if (sale_price_pkr == null || Number(sale_price_pkr) < 0) {
    return res.status(400).json({ error: 'sale_price_pkr must be a non-negative number' });
  }
  ...
  try {
    ...
    const a = await client.query(`SELECT * FROM ${cfg.table} WHERE id=$1 FOR UPDATE`, [asset_id]);
    const asset = a.rows[0];
    if (!asset) { await client.query('ROLLBACK'); return res.status(404).json({ error: 'Asset not found' }); }
    if (asset.status === 'sold') { await client.query('ROLLBACK'); return res.status(409).json({ error: 'Asset is already sold' }); }

    const buyer = await client.query('SELECT full_name FROM employees WHERE id=$1', [buyer_employee_id]);
    if (!buyer.rows[0]) { await client.query('ROLLBACK'); return res.status(404).json({ error: 'Buyer employee not found' }); }

    const label = assetLabel(asset);
    const fromEmployeeId = cfg.hasAssignee ? (asset.assigned_user_id || null) : null;
    ...
    const purchase = await client.query(
      `INSERT INTO asset_purchases
       ...
       RETURNING *`,
      [
        asset_type, asset_id, label, buyer_employee_id, buyer.rows[0].full_name,
        sale_price_pkr, book_value_pkr ?? asset.purchase_price_pkr ?? null, sale_date || null,
        invoice_number || null, payment_reference || null, notes || null, req.user.id,
      ],
    );
    ...
    await client.query(
      ...
      [asset_type, asset_id, label, fromEmployeeId, asset.status || null, notes || null, req.user.id],
    );
    ...
    if (cfg.hasAssignee) {
      ...
      await client.query(`UPDATE ${cfg.table} SET status='sold', updated_at=NOW() WHERE id=$1`, [asset_id]);
    }

    await client.query('COMMIT');

    await logActivity(
      req.user.id, 'BUYOUT', asset_type, asset_id, label,
      `Sold to ${buyer.rows[0].full_name} for PKR ${sale_price_pkr}`, getIP(req),
    );
    res.status(201).json(purchase.rows[0]);
  } catch (e) {
    await client.query('ROLLBACK');
    // Unique-index violation => asset already has a buyout row.
    if (e.code === '23505') return res.status(409).json({ error: 'Asset is already sold' });
    next(e);
  } finally {
    client.release();
  }
