  ...
  async (req, res, next) => {
    ...
    try {
      ...
      const reqRow = await client.query(
        "SELECT * FROM inv_requests WHERE id=$1 FOR UPDATE",
        [req.params.id],
      );
      if (!reqRow.rows[0]) {
        await client.query("ROLLBACK");
      ...
      if (!["submitted", "in_review"].includes(reqRow.rows[0].status)) {
        ...
        return res
          .status(400)
          .json({ error: "Request cannot be reviewed in current status" });
      }
      ...
      for (const d of decisions || []) {
        await client.query(
         ...
         WHERE id=$4 AND request_id=$5`,
          [
            d.action,
            d.action === "approved" ? d.qty_approved || 0 : 0,
      ...
      const items = await client.query(
        "SELECT id, item_id, item_status, qty_approved FROM inv_request_items WHERE request_id=$1",
        [req.params.id],
      );
      ...
      if (!allRejected) {
        for (const item of all.filter(
          (i) => i.item_status === "approved" && i.qty_approved > 0,
        )) {
          await client.query(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      if (!["approved", "partially_approved"].includes(reqRow.rows[0].status)) {
        ...
        return res
          .status(400)
          .json({ error: "Request must be approved before fulfillment" });
      }
      ...
      for (const item of approvedItems.rows) {
        ...
        if (!stock || stock.qty_available < item.qty_approved) {
          ...
          return res
            .status(400)
            .json({ error: `Insufficient stock for: ${item.item_name}` });
        }
        await client.query(
        ...
        await client.query(
         ...
         VALUES ($1,'assignment',$2,'inv_requests',$3,$4,$5)`,
          [
            ...
            -item.qty_approved,
            req.params.id,
            `Fulfilled for ${reqRow.rows[0].req_number}`,
            req.user.id,
          ],
        );
      }
...
router.post("/:id/cancel", requireAuth, async (req, res, next) => {
  try {
    const r = await db.query("SELECT * FROM inv_requests WHERE id=$1", [
      req.params.id,
    ]);
    if (!r.rows[0]) return res.status(404).json({ error: "Not found" });
    const row = r.rows[0];
    ...
    await db.query(
      "UPDATE inv_requests SET status='cancelled', updated_at=NOW() WHERE id=$1",
      [req.params.id],
    );
    await log(
      req.user.id,
