...
async function log(userId, action, id, label, details, ip) {
  await db.query(
    ...
    [userId, action, 'users', id, label, details, ip || null]
  );
}
...
// ── ACTIVITY LOG ──────────────────────────────────────────
router.get('/activity/log', requireAuth, adminOnly, async (req, res) => {
  try {
    const r = await db.query(`
...
router.get('/:id/permissions', requireAuth, adminOnly, async (req, res) => {
  try {
    const r = await db.query(
    ...
    for (const mod of MODULES) {
      if (!perms[mod]) perms[mod] = { can_create:false, can_read:true, can_update:false, can_delete:false };
    }
    res.json(perms);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.put('/:id/permissions', requireAuth, adminOnly, async (req, res) => {
  try {
    ...
    await db.query('DELETE FROM user_permissions WHERE user_id=$1', [req.params.id]);
...
router.post('/', requireAuth, adminOnly, async (req, res) => {
  ...
  if (!employee_id || !password || !role)
    return res.status(400).json({ error: 'employee_id, password and role are required' });
  if (password.length < 6)
    return res.status(400).json({ error: 'Password must be at least 6 characters' });
  if (!['super_admin','user'].includes(role))
    return res.status(400).json({ error: 'Role must be super_admin or user' });
  try {
    const empR = await db.query('SELECT * FROM employees WHERE id=$1', [employee_id]);
    const emp  = empR.rows[0];
    if (!emp)  return res.status(404).json({ error: 'Employee not found' });
    if (!emp.email) return res.status(400).json({ error: 'Employee has no email address — add one first' });
    if (emp.portal_user_id) return res.status(409).json({ error: 'This employee already has a portal account' });
    ...
    await log(req.user.id, 'created', newUser.id, newUser.email, `Role: ${role}`, getIP(req));
    res.status(201).json(newUser);
  } catch (err) {
    if (err.code === '23505') return res.status(409).json({ error: 'This email is already a portal user' });
...
router.put('/:id', requireAuth, adminOnly, async (req, res) => {
  try {
    ...
    if (!r.rows[0]) return res.status(404).json({ error: 'Not found' });
    await log(req.user.id, 'updated', r.rows[0].id, r.rows[0].email, `Role: ${role}`, getIP(req));
    res.json(r.rows[0]);
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// ── ADMIN RESET PASSWORD ──────────────────────────────────
router.patch('/:id/password', requireAuth, adminOnly, async (req, res) => {
  try {
    const { new_password } = req.body;
    if (!new_password || new_password.length < 6)
      return res.status(400).json({ error: 'Password must be at least 6 characters' });
    const hash = await bcrypt.hash(new_password, 10);
    const r = await db.query('UPDATE users SET password_hash=$1 WHERE id=$2 RETURNING email', [hash, req.params.id]);
    if (!r.rows[0]) return res.status(404).json({ error: 'Not found' });
    await log(req.user.id, 'password_reset', Number(req.params.id), r.rows[0].email, 'Admin reset password', getIP(req));
    res.json({ ok: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.delete('/:id', requireAuth, adminOnly, async (req, res) => {
  try {
    ...
    const r = await db.query('DELETE FROM users WHERE id=$1 RETURNING *', [req.params.id]);
    if (!r.rows[0]) return res.status(404).json({ error: 'Not found' });
    await log(req.user.id, 'deleted', r.rows[0].id, r.rows[0].email, 'User deleted', getIP(req));
    res.json({ ok: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
