...
async function log(userId, action, id, label, details, changes) {
  await logActivity(userId, action, "inv_assignments", id, label, details, null, changes);
}
...
router.get('/', requireAuth, async (req, res, next) => {
  try {
    ...
    else { sql += ` AND a.status != 'fully_returned'`; }
    if (employee_id) { params.push(employee_id); sql += ` AND a.assignee_id = $${params.length}`; }
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (!s.rows[0] || s.rows[0].qty_available < item.qty) {
        const ir = await client.query('SELECT name FROM inv_items WHERE id=$1', [item.item_id]);
        throw new Error(`Insufficient stock for: ${ir.rows[0]?.name || item.item_id}`);
      }
      await client.query(
    ...
    const asnRow = await client.query(
      ...
      [asnNum, assignee_id, req.user.id, expected_return_date || null, notes || null]
    );
    ...
    await log(req.user.id, 'ASSIGN', asn.id, asnNum, `Direct assignment to employee #${assignee_id}`);
    res.json(asn);
  } catch (e) {
...
router.post('/:id/return', requireAuth, perm('inventory', 'update'), async (req, res, next) => {
  ...
  try {
    ...
    const asn = await client.query('SELECT * FROM inv_assignments WHERE id=$1 FOR UPDATE', [req.params.id]);
    ...
    for (const item of items) {
      ...
      await client.query(
        `INSERT INTO inv_return_items (return_id, assignment_item_id, qty, condition, back_to_stock, notes)
         VALUES ($1,$2,$3,$4,$5,$6)`,
        [ret.id, aItem.id, item.qty || aItem.qty, item.condition || 'good', backToStock, item.notes || null]
      );

      await client.query(
        `UPDATE inv_assignment_items SET status=$1, returned_at=NOW(), return_condition=$2
         WHERE id=$3`,
        [item.condition === 'good' ? 'returned' : item.condition, item.condition, aItem.id]
      );
      ...
      await client.query(
        ...
        [aItem.item_id,
         ...
         ret.id, `Return: ${item.condition}`, req.user.id]
      );
    ...
    const newStatus = parseInt(remaining.rows[0].count) === 0 ? 'fully_returned' : 'partially_returned';
    await client.query(
    ...
    await client.query('COMMIT');
    await log(req.user.id, 'RETURN', parseInt(req.params.id), asn.rows[0].asn_number,
