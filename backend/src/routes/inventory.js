...
router.post('/categories', requireAuth, perm('inventory', 'create'), async (req, res) => {
  try {
    ...
    const r = await db.query(
      ...
      [name, parent_id || null, description || null, icon || 'package', sort_order || 0]
    );
...
router.put('/categories/:id', requireAuth, perm('inventory', 'update'), async (req, res) => {
  try {
    ...
    const r = await db.query(
      ...
      [name, parent_id || null, description || null, icon || 'package', sort_order || 0, is_active !== false, req.params.id]
    );
...
router.get('/items', requireAuth, async (req, res) => {
  try {
    ...
    if (tracking_type) { params.push(tracking_type); sql += ` AND i.tracking_type = $${params.length}`; }
    sql += ' ORDER BY i.name';
...
router.post('/items', requireAuth, perm('inventory', 'create'), async (req, res) => {
  ...
  try {
    ...
    const { name, category_id, description, model, manufacturer, sku, tracking_type, unit, initial_qty, reorder_level, reorder_qty } = req.body;
    if (!name) return res.status(400).json({ error: 'Name is required' });
    const ir = await client.query(
      `INSERT INTO inv_items (name, category_id, description, model, manufacturer, sku, tracking_type, unit, created_by)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,
      [name, category_id || null, description || null, model || null, manufacturer || null,
       sku || null, tracking_type || 'quantity', unit || 'pcs', req.user.id]
    );
    ...
    await client.query(
      `INSERT INTO inv_stock (item_id, qty_available, reorder_level, reorder_qty)
       VALUES ($1,$2,$3,$4)`,
      [item.id, qty, parseInt(reorder_level) || 5, parseInt(reorder_qty) || 10]
    );
    if (qty > 0) {
      await client.query(
    ...
    await log(req.user.id, 'CREATE', item.id, name, `Item created, initial qty: ${qty}`);
    res.json(item);
  } catch (e) {
...
router.put('/items/:id', requireAuth, perm('inventory', 'update'), async (req, res) => {
  try {
    ...
    const r = await db.query(
      ...
      [name, category_id || null, description || null, model || null, manufacturer || null,
       sku || null, tracking_type || 'quantity', unit || 'pcs', req.params.id]
    );
    if (!r.rows[0]) return res.status(404).json({ error: 'Not found' });
...
router.post('/items/:id/adjust', requireAuth, perm('inventory', 'update'), async (req, res) => {
  ...
  try {
    ...
    if (!change) return res.status(400).json({ error: 'qty_change is required' });
    const allowed = ['purchase','correction','damaged','lost','retired'];
    if (!allowed.includes(type)) return res.status(400).json({ error: 'Invalid type' });

    const sr = await client.query('SELECT * FROM inv_stock WHERE item_id=$1 FOR UPDATE', [req.params.id]);
    ...
    await client.query(`UPDATE inv_stock SET ${update}, updated_at=NOW() WHERE item_id=$1`, [req.params.id]);
    await client.query(
      `INSERT INTO inv_adjustments (item_id, type, qty_change, notes, performed_by)
       VALUES ($1,$2,$3,$4,$5)`,
    ...
    await client.query('COMMIT');
    await checkAlerts(parseInt(req.params.id));
    const item = await db.query('SELECT name FROM inv_items WHERE id=$1', [req.params.id]);
    await log(req.user.id, 'STOCK_ADJUST', parseInt(req.params.id), item.rows[0]?.name, `${type}: ${change > 0 ? '+' : ''}${change}`);
    res.json({ ok: true });
  } catch (e) {
    await client.query('ROLLBACK');
    res.status(500).json({ error: e.message });
...
router.get('/stats', requireAuth, async (req, res) => {
  try {
    const [items, stock, alerts, pending] = await Promise.all([
      ...
      db.query("SELECT COUNT(*) FROM inv_requests WHERE status IN ('submitted','in_review','approved')"),
    ]);
    res.json({
