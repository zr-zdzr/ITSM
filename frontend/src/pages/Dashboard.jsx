...
const Tip = ({ active, payload, label }) => {
  ...
  return (
    <div
      className="itms-card px-3 py-2 shadow-lg"
...
function BarList({ rows, keyField, valueField = "n", colorHex = "#00AA2F" }) {
  const max = Math.max(
    ...(rows || []).map((r) => Number(r[valueField] || 0)),
    1,
  );
  return (
      ...
      {(rows || []).map((r, i) => {
        ...
        return (
          <div key={i} className="d-flex align-items-center gap-2">
            <span
              className="text-secondary text-truncate flex-shrink-0"
              style={{ fontSize: "0.75rem", width: 112 }}
              title={r[keyField]}
            >
              {r[keyField] || "—"}
            </span>
            ...
            >
              <div
                ...
                style={{ width: `${(val / max) * 100}%`, background: colorHex }}
              />
            </div>
            <span
              className="text-secondary text-end flex-shrink-0"
              style={{ fontSize: "0.75rem", width: 24 }}
            >
              {val}
            </span>
          </div>
...
function MiniTable({ columns, rows, fetching, path, navigate }) {
  ...
  if (!rows.length)
    return (
      <p className="text-secondary small mt-3 text-center py-3 mb-0">
        No records found
      </p>
    );
  return (
...
function SystemsContent({ data, navigate }) {
  ...
  const assignSlices = [
    {
      name: "Employees",
      value: Number(assignment?.employees || 0),
      fill: SYS_ASSIGN_COLORS.Employees,
    },
    {
      name: "WFH",
      value: Number(assignment?.wfh || 0),
    ...
    {
      name: "Damaged",
      value: Number(assignment?.damaged || 0),
      fill: SYS_ASSIGN_COLORS.Damaged,
    },
  ].filter((d) => d.value > 0);

  const locData = (byLocation || [])
    .slice(0, 8)
    .map((r) => ({ name: r.location || "Unknown", n: Number(r.n) }));
  const genData = (byGeneration || [])
    .slice(0, 8)
    .map((r) => ({ name: r.generation || "Unknown", n: Number(r.n) }));
  const typeData = (byType || []).map((r) => ({
    name: r.type === "System" ? "PC/Desktop" : r.type || "Other",
    n: Number(r.n),
  }));
    ...
    <>
        ...
        <div className="col-12 col-sm-6">
          <div className="row g-2">
            {[
              {
                label: "Employees",
                value: assignment?.employees,
                color: "#00AA2F",
                bg: "rgba(0,170,47,0.1)",
              },
              {
                label: "WFH",
                value: assignment?.wfh,
                color: "#0ea5e9",
      ...
      <div className="row g-4 pt-3 mt-0 border-top">
        <div className="col-12 col-sm-4">
          <p
            className="text-secondary text-uppercase mb-2 d-flex align-items-center gap-1"
            style={{ fontSize: "10px", letterSpacing: "0.08em" }}
          >
            <Monitor size={10} /> By Type
          </p>
          {typeData.length > 0 ? (
        ...
        <div className="col-12 col-sm-4">
          <p
            ...
            style={{ fontSize: "10px", letterSpacing: "0.08em" }}
          >
            <MapPin size={10} /> By Location
          </p>
          {locData.length > 0 ? (
            <ResponsiveContainer
              width="100%"
              height={Math.max(100, locData.length * 22)}
            >
              <BarChart
                data={locData}
                layout="vertical"
        ...
        <div className="col-12 col-sm-4">
          <p
            ...
            style={{ fontSize: "10px", letterSpacing: "0.08em" }}
          >
            <Cpu size={10} /> By Generation
          </p>
          {genData.length > 0 ? (
            <ResponsiveContainer
              width="100%"
              height={Math.max(100, genData.length * 22)}
            >
              <BarChart
                data={genData}
                layout="vertical"
  ...
  const assignSlices = [
    {
      name: "Employees",
      value: Number(assignment?.employees || 0),
      fill: MOB_ASSIGN_COLORS.Employees,
    },
    {
      name: "WFH",
      value: Number(assignment?.wfh || 0),
    ...
    },
    {
      name: "Damaged",
      value: Number(assignment?.damaged || 0),
      fill: MOB_ASSIGN_COLORS.Damaged,
    },
  ].filter((d) => d.value > 0);
      ...
      <div className="row g-4 pt-3 mt-0 border-top">
        <div className="col-12 col-sm-6">
          ...
          {locData.length > 0 ? (
            <ResponsiveContainer
              width="100%"
              height={Math.max(80, locData.length * 22)}
            >
              <BarChart
                data={locData}
                layout="vertical"
  ...
  const assignSlices = [
    {
      name: "Employees",
      value: Number(assignment?.employees || 0),
      fill: SIM_ASSIGN_COLORS.Employees,
    },
    {
      name: "WFH",
      value: Number(assignment?.wfh || 0),
      fill: SIM_ASSIGN_COLORS.WFH,
    },
    {
      name: "Services",
      value: Number(assignment?.for_services || 0),
      fill: SIM_ASSIGN_COLORS.Services,
    },
  ].filter((d) => d.value > 0);
          ...
          {[
            ...
            >
              <div className="d-flex align-items-center gap-2">
                ...
                />
                <span
                  style={{ fontSize: "0.75rem", color: s.color, opacity: 0.8 }}
                >
                  {s.label}
                </span>
              </div>
...
function EmployeesContent({ data, navigate }) {
  ...
  const permanentCount =
    typeSlices.find((t) => t.name === "Permanent")?.value ?? 0;
  const contractualCount =
    typeSlices.find((t) => t.name === "Contractual")?.value ?? 0;
        ...
        {[
          ...
          <div key={i} className="col-6 col-md-3">
            <div className="rounded-3 px-3 py-2" style={{ background: s.bg }}>
              ...
              <span
                style={{ fontSize: "0.75rem", color: s.color, opacity: 0.85 }}
              >
                {s.label}
              </span>
            </div>
            ...
            <>
              ...
              <div className="d-flex flex-column gap-1">
                {typeSlices.map((s, i) => (
                  ...
                  >
                    <span style={{ fontSize: "0.75rem", color: s.fill }}>
                      {s.name}
                    </span>
                    <span
                      className="fw-bold"
                      style={{ fontSize: "0.875rem", color: s.fill }}
                    >
                      {s.value}
                    </span>
                  </div>
...
function CloudContent({ gws, gwsLicense, gwsType }) {
  const total = (gws || []).reduce((s, r) => s + Number(r.n), 0);
  const users = Number(
    (gwsType || []).find((r) => r.account_type === "user")?.n || 0,
  );
  const services = Number(
    (gwsType || []).find((r) => r.account_type === "service_account")?.n || 0,
  );
  const active = Number((gws || []).find((r) => r.status === "active")?.n || 0);
  const suspended = Number(
    (gws || []).find((r) => r.status === "suspended")?.n || 0,
  );
  const licMax = Math.max(...(gwsLicense || []).map((r) => Number(r.n)), 1);
  ...
  return (
    <>
      ...
      <div className="border-top pt-4">
        <p
          className="text-secondary text-uppercase mb-3 d-flex align-items-center gap-1"
          style={{ fontSize: "10px", letterSpacing: "0.08em" }}
        >
          <Cloud size={10} /> License Distribution
        </p>
        {(gwsLicense || []).length > 0 ? (
          <div className="d-flex flex-column gap-2">
            {(gwsLicense || []).map((r, i) => {
              const color = LIC_COLORS[r.license] || "#a1a1aa";
              return (
                <div key={i} className="d-flex align-items-center gap-2">
                  <span
                    className="text-secondary flex-shrink-0"
          ...
          <p className="text-secondary small mb-0">No data yet</p>
        )}
      </div>
    </>
  );
  ...
  return (
    <div className="itms-card overflow-hidden h-100 d-flex flex-column">
      <div className="d-flex align-items-center gap-2 px-3 py-2 border-bottom flex-shrink-0">
        ...
        <h3 className="mb-0 fw-semibold" style={{ fontSize: "0.875rem" }}>
          Last 24 Hours
        </h3>
        {!loading && (
          <span
            className="badge bg-secondary bg-opacity-25 text-secondary ms-1"
            style={{ fontSize: "11px" }}
          >
      ...
      {loading ? (
        ...
        <div className="px-3 py-5 text-center text-secondary small">
          No activity in the last 24 hours
        </div>
      ) : (
        <div className="overflow-auto flex-grow-1" style={{ maxHeight: 280 }}>
          {entries.map((a, i) => {
            const badge = ACTION_BADGE[a.action] || {
              ...
              >
                <div
                  ...
                  style={{
                    width: 6,
                    height: 6,
                    background: ACTION_DOT[a.action] || "#71717a",
                  }}
                />
                <div className="flex-grow-1 min-w-0 d-flex align-items-start flex-wrap gap-1">
                  <span className="small fw-medium flex-shrink-0">
                    {a.user_name || a.user_email || "Unknown"}
                  </span>
                  <span
                    className="rounded flex-shrink-0 px-1"
                    style={{
                      fontSize: "10px",
                      background: badge.bg,
                      color: badge.color,
                  ...
                  {a.table_name && (
                    <span
                      ...
                      style={{ fontSize: "10px" }}
                    >
                      {MODULE_LABEL[a.table_name] || a.table_name}
                    </span>
                  )}
                  {a.record_label && (
                    <span
                ...
                <span
                  className="text-secondary flex-shrink-0 mt-0"
        ...
        <div className="d-flex align-items-center gap-4 small">
          {stats.out_of_stock > 0 && (
            <span className="d-flex align-items-center gap-1 text-danger">
              ...
              <strong>{stats.out_of_stock}</strong> item
              {stats.out_of_stock !== 1 ? "s" : ""} out of stock
            </span>
          )}
          {stats.low_stock > 0 && (
            <span className="d-flex align-items-center gap-1 text-warning">
              <AlertTriangleIcon size={13} />
              <strong>{stats.low_stock}</strong> item
              {stats.low_stock !== 1 ? "s" : ""} low on stock
            </span>
          )}
        </div>
      )}
...
export default function Dashboard() {
  ...
  const load = useCallback(() => {
    ...
    Promise.all([
      ...
      canPerm("inventory", "read")
        ? api.get("/api/inventory/stats").catch(() => null)
  ...
  useEffect(() => {
    load();
    const handler = (e) => {
      if (e.detail?.action === "refresh") load();
    };
  ...
  const networkTotal = (data?.networkDevices || []).reduce(
    (a, r) => a + Number(r.n),
    0,
  );
  ...
  const netByType = (data?.networkDevices || []).map((r) => ({
    name: r.device_type,
    value: Number(r.n),
  }));
  ...
  return (
    <div className="d-flex flex-column gap-4">
      ...
      >
        ...
        <div className="d-flex align-items-center justify-content-between gap-3">
          ...
          <div className="d-flex align-items-center gap-2">
            ...
            <div
              className="d-none d-sm-flex align-items-center justify-content-center flex-shrink-0 rounded-3"
              style={{
                width: 48,
                height: 48,
      ...
      {canPerm("gws", "read") && (
        <ContentPanel open={openSection === "gws"} loading={loading}>
          <CloudContent
            gws={data?.gws}
            gwsLicense={data?.gwsLicense}
            gwsType={data?.gwsType}
          />
        </ContentPanel>
      )}
      {canPerm("inventory", "read") && invStats && (
        <ContentPanel open={openSection === "inventory"} loading={loading}>
          <InventoryContent
            stats={invStats}
            rows={sectionRows["inventory"] || []}
            rowsLoading={sectionLoading["inventory"] || false}
            navigate={navigate}
          />
        </ContentPanel>
      )}
      ...
      <div className="row g-4">
        ...
        <div
          className={cn("col-12", canPerm("network", "read") ? "col-lg-6" : "")}
        >
          <motion.div
            initial={{ opacity: 0, y: 6 }}
            animate={{ opacity: 1, y: 0 }}
      ...
      {canPerm("systems", "read") &&
        data &&
        (data.warrantyExpired > 0 || data.warrantySoon > 0) && (
          <motion.div
            initial={{ opacity: 0, y: 6 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.23 }}
            className="itms-card p-4"
          >
            <div className="d-flex align-items-center gap-2 mb-3">
            ...
            <div className="row g-3">
              ...
              <div className="col-12 col-sm-6">
                ...
                >
                  <span className="small" style={{ color: "#fcd34d" }}>
                    Expiring in 90 days
                  </span>
                  <span
                    className="fw-bold"
                    style={{ fontSize: "1.1rem", color: "#f59e0b" }}
                  >
                    {data.warrantySoon}
                  </span>
                </div>
