...
const config = {
  ...
  qrData: row => {
    const tag = row.asset_tag || genAssetTag(row.purchase_date, 'ID') || `${row.brand||''} ${row.model||''}`.trim() || 'Network Device'
    return {
      label: tag,
  ...
  fields: [
    { name: 'asset_tag',  label: 'Asset Tag',    type: 'text', placeholder: 'Auto-generated from purchase date' },
