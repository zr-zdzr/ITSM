    ...
    <div className="row g-3" style={{ maxHeight: '65vh', overflowY: 'auto', paddingRight: 4 }}>
      ...
      <div className="col-md-6">
        <Fld label="Asset Tag" required>
          <input className={inp} value={vals.asset_tag || ''} onChange={e => set('asset_tag', e.target.value)} placeholder="IT-MB-0001" />
        </Fld>
      </div>
      ...
      <div className="col-md-6">
        <Fld label="Manufacturer" required>
          <input className={inp} value={vals.manufacturer || ''} onChange={e => set('manufacturer', e.target.value)} placeholder="Samsung, Apple, Xiaomi…" />
        </Fld>
      </div>

      <div className="col-md-6">
        <Fld label="Model" required>
          <input className={inp} value={vals.model || ''} onChange={e => set('model', e.target.value)} placeholder="Galaxy S23, iPhone 14…" />
        </Fld>
      </div>

      <div className="col-12">
        <Fld label="Serial No." required half={false}>
          <input className={inp} value={vals.serial_number || ''} onChange={e => set('serial_number', e.target.value)} onBlur={e => set('serial_number', e.target.value.toUpperCase())} placeholder="Device serial number" autoCapitalize="characters" style={{ textTransform: 'uppercase' }} />
        </Fld>
      </div>
      ...
      <div className="col-md-6">
        <Fld label="IMEI 1">
          <input className={inp} value={vals.imei1 || ''} onChange={e => set('imei1', e.target.value)} onBlur={e => set('imei1', e.target.value.toUpperCase())} placeholder="15-digit IMEI" autoCapitalize="characters" style={{ textTransform: 'uppercase' }} />
        </Fld>
      </div>

      <div className="col-md-6">
        <Fld label="IMEI 2">
          <input className={inp} value={vals.imei2 || ''} onChange={e => set('imei2', e.target.value)} onBlur={e => set('imei2', e.target.value.toUpperCase())} placeholder="Dual-SIM IMEI" autoCapitalize="characters" style={{ textTransform: 'uppercase' }} />
        </Fld>
      </div>

      <div className="col-md-6">
        <Fld label="OS">
          <select className={sel} value={vals.os || ''} onChange={e => set('os', e.target.value)}>
...
function MobileDeviceView(row) {
  const assignLabel = {
    employee: 'Employee', user: 'Employee', wfh: 'WFH', inventory: 'Inventory', damaged: 'Damaged'
  }
  const purposeLabel = { official: 'Official', service: 'Service', personal: 'Personal', qa_testing: 'QA Testing' }
  ...
  return (
    <dl className="row g-3">
      ...
      <Field label="Assigned To"  value={assignLabel[row.assigned_type] || row.assigned_type} />
      ...
      <Field label="Warranty Expiry" value={row.warranty_expiry ? new Date(row.warranty_expiry).toLocaleDateString() : null} />
      {row.notes && (
        <div className="col-12">
...
const config = {
  ...
  qrData: row => {
    const tag = row.asset_tag || genAssetTag(row.purchase_date, 'MB') || `${row.manufacturer || ''} ${row.model || ''}`.trim() || 'Mobile Device'
    return {
      label: tag,
      ...
      details: [
        ...
        row.assigned_user_name ? `Assigned: ${row.assigned_user_name}` : row.assigned_type === 'inventory' ? 'Inventory' : row.assigned_type,
      ].filter(Boolean),
    }
  },
  ...
  columns: [
    ...
    { key: 'assigned_type',     label: 'Assigned To',  render: (_, row) => <AssignedBadge row={row} /> },
    { key: 'department',        label: 'Department' },
    { key: 'type',              label: 'Type',         sortable: true },
    { key: 'manufacturer',      label: 'Manufacturer', sortable: true },
    { key: 'model',             label: 'Model' },
    { key: 'serial_number',     label: 'Serial No.',  render: v => v ? v.toUpperCase() : '—' },
    { key: 'imei1',             label: 'IMEI 1',      render: v => v ? v.toUpperCase() : '—' },
    { key: 'imei2',             label: 'IMEI 2',      render: v => v ? v.toUpperCase() : '—' },
    { key: 'location',          label: 'Location' },
    { key: 'purpose',           label: 'Purpose',      render: v => v ? (v === 'official' ? 'Official' : v === 'service' ? 'Service' : v) : '—' },
    { key: 'warranty_expiry',   label: 'Warranty',     render: v => v ? new Date(v).toLocaleDateString() : '—' },
    { key: 'notes',             label: 'Note',         render: v => v ? <span className="text-secondary small text-truncate d-block" style={{ maxWidth: 120 }} title={v}>{v}</span> : '—' },
  ],
  ...
  renderForm: (vals, setVals) => <MobileDeviceForm vals={vals} setVals={setVals} />,
