  ...
  useEffect(() => {
    ...
    api
      ...
      .then((d) => {
        ...
        setEmployees(list);
        // Auto-fill department for existing records that already have an employee selected
    ...
    >
      ...
      <div className="col-md-6">
        <Fld label="Type" required>
          ...
          >
            ...
            {["Laptop", "PC", "Server", "Workstation", "Other Device"].map(
              (t) => (
      ...
      <div className="col-md-6">
        <Fld label="Brand">
          <select
            className={sel}
            value={vals.brand_type || ""}
            onChange={(e) => set("brand_type", e.target.value)}
          >
      ...
      <div className="col-md-6">
        <Fld label="Serial No." required>
          <input
            ...
            placeholder="SN from BIOS/label"
      ...
      <div className="col-md-6">
        <Fld label="CPU 2 — Model (optional)">
          <input
            className={inp}
            value={vals.cpu2 || ""}
            onChange={(e) => set("cpu2", e.target.value)}
            placeholder="Intel Xeon E5-2630"
          />
        </Fld>
      </div>
      <div className="col-md-6">
        <Fld label="CPU 2 — Cores">
          <input
            className={inp}
            value={vals.cpu2_cores || ""}
            onChange={(e) => set("cpu2_cores", e.target.value)}
            placeholder="8"
          />
        </Fld>
      </div>
            ...
            <div className="row g-2">
              <div className="col-3">
                <Fld label={`Slot ${n} — Size (GB)`} half={false}>
                  <input
                    className={inp}
                    value={vals[`ram${n}_size`] || ""}
                    onChange={(e) => set(`ram${n}_size`, e.target.value)}
                    placeholder="8"
                  />
                </Fld>
              </div>
              <div className="col-3">
                <Fld label="Bus" half={false}>
                  <input
                    className={inp}
                    value={vals[`ram${n}_bus`] || ""}
                    onChange={(e) => set(`ram${n}_bus`, e.target.value)}
                    placeholder="3200MHz"
                  />
                </Fld>
              </div>
              <div className="col-3">
                <Fld label="Slot No." half={false}>
                  <input
                    className={inp}
                    value={vals[`ram${n}_slot`] || ""}
                    onChange={(e) => set(`ram${n}_slot`, e.target.value)}
                    placeholder="A1"
                  />
                </Fld>
              </div>
              <div className="col-3">
                <Fld label="Serial No." half={false}>
                  <input
                    className={inp}
                    value={vals[`ram${n}_serial`] || ""}
                    onChange={(e) => set(`ram${n}_serial`, e.target.value)}
                    placeholder="SN…"
                  />
                </Fld>
              </div>
        ...
        <React.Fragment key={n}>
          <div className="col-md-6">
            <Fld label={`Disk ${n} — Type`} half={false}>
              <select
                className={sel}
                value={vals[`disk${n}_type`] || ""}
                onChange={(e) => set(`disk${n}_type`, e.target.value)}
              >
                <option value="">— Select —</option>
                {["SATA", "SSD", "NVMe", "NVMe2", "SATA SSD"].map((t) => (
                  <option key={t}>{t}</option>
                ))}
              </select>
            </Fld>
          ...
          <div className="col-md-6">
            <Fld label={`Disk ${n} — Size`} half={false}>
              <input
                className={inp}
                value={vals[`disk${n}_size`] || ""}
                onChange={(e) => set(`disk${n}_size`, e.target.value)}
      ...
      <div className="col-12">
        <Fld label="Purpose of Use" half={false}>
          <input
            ...
            placeholder="Daily use, Development, Server…"
          />
        </Fld>
      </div>
      ...
      <div className="col-md-6">
        <Fld label="Status">
          <select
            className={sel}
            value={vals.status || "available"}
            onChange={(e) => set("status", e.target.value)}
          >
            {["available", "assigned", "repair", "retired", "lost"].map((s) => (
      ...
      <div className="col-md-6">
        <Fld label="Condition">
          ...
          >
            <option value="">— Select —</option>
            {["Working", "Good", "Fair", "Damaged", "Under Repair"].map((c) => (
...
function SystemDeviceView({ row }) {
  ...
  const diskCount = [1, 2, 3].filter(
    (n) => row[`disk${n}_size`] || row[`disk${n}_type`],
  ).length;
      ...
      {(row.cpu || row.cpu2) && (
        <div className="pt-3 border-top">
          ...
          <div className="row g-2 small">
            {row.cpu && (
              <div className="col-6">
                <span className="text-secondary">CPU 1:</span> {row.cpu}
                {row.cpu_cores ? ` (${row.cpu_cores} cores)` : ""}
              </div>
            )}
            {row.cpu2 && (
              <div className="col-6">
                <span className="text-secondary">CPU 2:</span> {row.cpu2}
                {row.cpu2_cores ? ` (${row.cpu2_cores} cores)` : ""}
              </div>
            )}
          </div>
        </div>
            ...
            {[1, 2, 3, 4]
              ...
              .map((n) => (
                ...
                >
                  ...
                  {row[`ram${n}_bus`] && (
                    <span className="text-secondary">{row[`ram${n}_bus`]}</span>
                  )}
                  {row[`ram${n}_slot`] && (
...
function AssignedBadge({ row }) {
  ...
  if (row.assigned_type === "wfh")
    return (
      <span>
        <span
          className="badge me-1 badge-assign-wfh"
          style={{ fontSize: "10px" }}
        >
          WFH
        </span>
        {row.assigned_user_name && (
          <span className="text-secondary small">{row.assigned_user_name}</span>
        )}
      </span>
    );
  ...
  if (row.assigned_type === "damaged")
    return (
      <span className="badge badge-assign-damaged" style={{ fontSize: "10px" }}>
        Damaged
      </span>
    );
...
const config = {
  ...
  renderForm: (vals, setVals) => (
    <SystemDeviceForm vals={vals} setVals={setVals} />
  ),
